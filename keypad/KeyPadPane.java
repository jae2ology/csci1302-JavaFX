...
public class KeyPadPane extends GridPane {
	...
	protected void registerEventHandlers() {
		...
		ArrayList<Button> currList = new ArrayList<>();
		...
		for (int i = 0; i < currList.size(); i++) {
			currList.get(i).setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent e) {
					...
					// method to document the button being clicked for the phone number
					Button clickedButton = (Button)e.getSource();
					String value = clickedButton.getText().trim();
					typedNumber.append(value);
	...
	private StringBuilder typedNumber = new StringBuilder(); // stringbuilder to collect number input
	public String getTypedNumber() {
		return typedNumber.toString(); // getter method
	}

	public void clearTypedNumber() {
		typedNumber.setLength(0); // clears the label in the main method
	}
